import { ADD_TODO, DELETE_TODO, STATUS, TODO_ERROR, TODO_REQUEST, TODO_SUCCESS, EDIT_TODO } from "./types";
...
const TodoReducer = (state = initialState, action) => {
    switch (action.type) {
        ...
        case STATUS:
            ...
            const changeStatus = state.list.find(list => (list.id === checkid));
            console.log("--list", changeStatus);
            const statusUpdated = { ...changeStatus, status: status }
            console.log("statslist", action.payload);
            const updateList = state.list.map(list => list.id === checkid ? statusUpdated : list);
